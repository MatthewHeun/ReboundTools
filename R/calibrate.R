...
#'                      in `.rebound_data`.
#' @param case,I_E See `ReboundTools::eeu_base_params`.
#' @param S_dot_dev See `ReboundTools::star_vars`.
#' @param N_dot_hat See `ReboundTools::hat_vars`.
#' @param Re_macro,Re_tot See `ReboundTools::rebound_terms`.
#'
...
calibrate_k <- function(.rebound_data, Re_tot_target, 
  ...
  .rebound_data[[S_dot_dev]] / .rebound_data[[N_dot_hat]] / .rebound_data[[I_E]] * 
    (.rebound_data[[Re_macro]] + Re_tot_target - .rebound_data[[Re_tot]]) %>% 
    magrittr::set_names(.rebound_data[[case]])
}