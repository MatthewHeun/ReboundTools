...
test_that("indifference_fun() picks up data from a data frame", {
  
  DF <- tibble::tribble(~Case, ~qs1_qs0, ~Co1_Co0, ~sigma, ~f_Cs,
                        "Case 1", 1,        1,        0.04,   0.01, 
                        "Case 2", 0.5,      0.7,      0.4,    0.00001)
  g <- ggplot2::ggplot() +
    ggplot2::stat_function(data = data.frame(x = c(0.1, 10)),
                           mapping = ggplot2::aes(x = x), 
                           fun = indifference_func, 
