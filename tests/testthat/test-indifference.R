...
test_that("add_indifference_curve() works as expected", {
  ...
  res <- add_indifference_curve(meta = meta, graph_type = "Test type", 
                                ...
                                qs1_qs0 = 5, Cg1_Cg0 = 10, qs2_qs0 = 7, f_Cs_orig = 0.1, sigma = 0.04)
  ...
  expect_equal(res$linewidth %>% unique(), 0.5)
  expect_equal(res$linetype %>% unique(), "solid")
  expect_equal(res$qs1_qs0 %>% unique(), 5)
  expect_equal(res$Cg1_Cg0 %>% unique(), 10)
  ...
  res2 <- add_indifference_curve(meta = meta2, graph_type = c("Consumption", "Consumption"), 
                                 ...
                                 qs1_qs0 = c(1.5,1.6), Cg1_Cg0 = c(0.999,0.998), 
...
test_that("indifference_func() works as expected", {
  expect_equal(indifference_func(qs_qs0 = 1, qs1_qs0 = 1, Cg1_Cg0 = 1, f_Cs = 0.25, sigma = 0.3), 1)
  # At point (1, 1), the result is independent of f_Cs and sigma.
  expect_equal(indifference_func(qs_qs0 = 1, qs1_qs0 = 1, Cg1_Cg0 = 1, f_Cs = 0.5, sigma = 0.3), 1)
  expect_equal(indifference_func(qs_qs0 = 1, qs1_qs0 = 1, Cg1_Cg0 = 1, f_Cs = 0.25, sigma = 0.8), 1)
  ...
  f_Cs_orig <- 0.01
  rho <- (sigma-1)/sigma
  u1_u0 <- (f_Cs_orig*(qs1_qs0)^rho + (1 - f_Cs_orig)*(Cg1_Cg0)^rho)^(1/rho)
  
  x <- 1
  y <- indifference_func(qs_qs0 = x, qs1_qs0 = qs1_qs0, Cg1_Cg0 = Cg1_Cg0, f_Cs_orig = f_Cs_orig, sigma = sigma)
  expect_equal(y, 1)
  x <- 0.5
  y <- indifference_func(qs_qs0 = x, qs1_qs0 = qs1_qs0, Cg1_Cg0 = Cg1_Cg0, f_Cs_orig = f_Cs_orig, sigma = sigma)
